...
Звук в браузере такой, какой есть. Бас слабый на ноутбучных динамиках, одно видео сведено слишком тихо, у другого резкие, утомляющие верхи — а большинство эквалайзеров-расширений либо игнорируют стриминговые сайты, либо полностью замолкают на них. Umbra EQ правит звук именно той вкладки, которую вы слушаете, вживую.

### Один звук на всех вкладках
Настройте эквалайзер один раз, и он применится к каждой вкладке, для которой вы его включите. Никакой возни с каждым сайтом — общий профиль работает везде по умолчанию, а вы просто меняете то, что слышите.

### Правила для сайтов, когда они нужны
Нужно один видеосайт сделать ярче, а остальное не трогать? Добавьте правило по имени хоста — оно переопределит общий звук для подходящих вкладок (побеждает первое совпадение), пока остальной веб сохраняет ваш общий профиль.

### Работает там, где другие молчат
Не только YouTube. Umbra работает на Netflix, Spotify и других стриминговых сайтах, где эквалайзеры-расширения часто не дают звука вовсе.

### Приватность по умолчанию
100% локально. Без аккаунта, без входа, ноль сетевых запросов, ноль аналитики. Ваш звук нигде не записывается и никуда не отправляется, а в рабочей сборке действует строгий CSP (`script-src 'self'; object-src 'self'`) — без удалённого кода и без `eval`.

### Чистый звук без щелчков
Ограничитель типа «кирпичная стена» гасит усиление до того, как оно начнёт клиппировать, а каждое изменение громкости идёт через `setTargetAtTime` — так что перемещение полосы никогда не даёт щелчков и треска.
...
npm test           # 46 модульных тестов Vitest для аудио, пресетов и правил
...
Manifest V3. **Попап** на React и TypeScript (Tailwind и shadcn/ui) рисует эквалайзер обычным SVG. **Попап — единственный источник истины**: он вычисляет звук каждой вкладки (правило → общий профиль → плоский) и отправляет полосы движку. **Аудиодвижок — на чистом JS**: service worker (`src/background.js`) владеет offscreen-документом и выдаёт id потоков захвата вкладок, а offscreen-документ (`public/offscreen.js`) — это простой применитель: он строит и держит цепочку из 11 biquad-фильтров на каждую захваченную вкладку (плавно, без щелчков, через `setTargetAtTime`) за ограничителем «кирпичная стена». Настройки, пресеты и правила лежат в `chrome.storage` и принадлежат попапу. Чистая математика аудио, пресетов и правил находится в `src/lib` и покрыта модульными тестами Vitest. Политика безопасности контента строгая (`script-src 'self'; object-src 'self'`), поэтому в рабочей сборке нет удалённого кода и нет `eval`.

Полный справочник по архитектуре — в [`PROJECT.md`](PROJECT.md), а как работать над проектом — в [`CONTRIBUTING.md`](CONTRIBUTING.md).
...
<p align="center">
  <a href="https://star-history.com/#skyjacc/Umbra&Date"><img src="https://api.star-history.com/svg?repos=skyjacc/Umbra&type=Date" alt="История звёзд"></a>
</p>
