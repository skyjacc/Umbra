...
**Почему Chrome пишет, что вкладкой делятся?** Umbra берёт звук вкладки через API захвата — тот же механизм, что и демонстрация экрана, поэтому Chrome показывает свой индикатор. Читается **только звук**: ни видео, ни содержимого страницы, и ничего не уходит с устройства. Индикатор ставит сам браузер, расширение отключить его не может — и это правильно: ты всегда знаешь, когда вкладку захватывают. Пропадает после «Стоп».
