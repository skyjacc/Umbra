...
- **Дата:** 2026-07-07 (первичный аудит; датированные записи в §8 ниже — последующие раунды)
- **Версия:** 2.2.0 · **Ветка:** main
- **Объём:** ~4200 строк (`src/` + `public/`), 3 тест-файла, 47 тестов
...
(`MediaStreamSource(tab) → preGain → 11×biquad → postGain → limiter → speakers`). Две вкладки
держат две разные кривые одновременно (фильм и музыка звучат по-разному).

- **Модель v2 — попап источник истины:** offscreen — тупой применитель (нет надёжного storage).
  Попап резолвит каждую вкладку (правило → общий профиль → flat) и пушит бэнды в движок.
- **Общий профиль:** `chrome.storage.local` `DEFAULT_EQ` — звук на всех вкладках без правила.
- **Правила:** паттерн адреса → пресет/кривая, первое совпадение выигрывает (`chrome.storage.sync`).
- **Пресеты:** `chrome.storage.sync` (синк между браузерами).
- Приватность: всё локально, ничего не пишется/не отправляется, аккаунта нет.
  (Старые v1-ключи `DEQ.<host>` / `AUTO_DOMAIN` — мёртвые leftover, не используются.)
